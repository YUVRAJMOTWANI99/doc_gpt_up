...
from src.utils import deactivate_pdf_view, upload_file_form, show_vector_store, clear_chat, extract_source_docs, \
    prepare_chat_history, post_inference_buttons, log_text_convo, load_embedding_model, choose_to_delete
...
def register_new_user() -> None:
    st.warning("Do not use HQ-credentials!")
    # Register new user
    try:
    ...
    st.button("Login\t", on_click=deactivate_register_user_view, type="primary")
    st.caption("Click login to go back")
...
def delete_doc():
    with st.sidebar:
        st.write("Choose files to delete")
        choose_to_delete()
...
if __name__ == "__main__":
    ...
    with c:
        ...
        if not do_security_check():
            ...
            with c2:
                st.button("Sign Up", on_click=activate_register_user_view, type="primary")
            st.stop()
